...
const Navbar = () => {
  ...
  const {user,setUser,isSeller,setshowUserLogin, navigate, searchQuery, setSearchQuery ,
...
<div className="hidden sm:flex items-center gap-8">
  ...
  {!user && !isSeller && (
    <button onClick={()=> navigate('/seller')}
     className="cursor-pointer px-8 py-2 border border-primary hover:bg-primary-dull transition text-black rounded-full">
      Seller Login
    </button>
  ) }
...
<div className={`${open ? 'flex' : 'hidden'}  z-50 absolute top-[60px] left-0 w-full bg-white shadow-md py-4 flex-col items-start gap-2 px-5 text-sm md:hidden`}>
    ...
    className="cursor-pointer px-6 py-2 mt-2 border border-primary  hover:bg-primary transition text-black rounded-full text-sm">
    Seller Login
    </button>:null}
